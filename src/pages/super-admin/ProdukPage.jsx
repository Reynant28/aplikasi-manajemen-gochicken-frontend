...
const ProdukPage = () => {
  ...
  const filteredProduk = produk.filter((prod) => {
    const matchesSearch =
      prod.nama_produk.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingProduk) {
        ...
        await axios.post(
          `${API_URL}/produk/${editingProduk.id_produk}`,
          submitData,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm sm:text-base">
              Kelola produk dan inventaris toko Anda
            </p>
          </div>
          <button
                ...
                <div className="flex items-center gap-4 p-4 bg-red-50 rounded-lg">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800">
                      {deleteProduk.nama_produk}
                    </p>
                    <p className="text-sm text-gray-500">
                      {formatRupiah(deleteProduk.harga)}
                    </p>
                  </div>
                </div>
