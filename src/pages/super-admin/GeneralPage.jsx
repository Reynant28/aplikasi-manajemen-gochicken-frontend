      ...
      <div className="bg-white/90 backdrop-blur-sm p-2 rounded-lg shadow-lg border border-gray-200">
        ...
        <p
          className={`text-sm font-semibold ${
            mode === "Pendapatan" ? "text-red-500" : "text-orange-500"
          }`}
        >
...
const DailySummaryWidget = ({ token }) => {
    ...
    const fetchRevenueBreakdown = async () => {
      ...
      try {
        ...
        if (res.data.status === "success") {
          // Process the revenue breakdown using the utility function
          const processedBreakdown = processRevenueBreakdown(
            res.data.data || []
          );
  ...
  const selectedData =
    summaries.find((s) => s.id_cabang == selectedBranchId) ||
    summaries.find((s) => s.id_cabang === "all");
    ...
    >
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 gap-2">
        ...
        <div className="relative w-full sm:w-48">
          ...
          >
            {summaries.map((s) => (
              <option key={s.id_cabang} value={s.id_cabang}>
                {s.nama_cabang}
              </option>
            ))}
          </select>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-sm">
                ...
                <p className="font-semibold text-green-600">
                  {formatRupiah(selectedData.pendapatan_hari_ini)}
                </p>
              </div>
              <div className="flex justify-between items-center text-sm">
                <p className="text-gray-600">Total Pengeluaran</p>
                <p className="font-semibold text-red-600">
                  {formatRupiah(selectedData.pengeluaran_hari_ini)}
                </p>
              </div>
            </div>
            <hr className="my-3" />
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      ...
      try {
        ...
        setChartData(
          processChartData(res.data.data[chartMode] || [], chartFilter)
        );
        //eslint-disable-next-line no-unused-vars
      } catch (err) {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-500 text-sm sm:text-base">
          Ringkasan performa untuk seluruh cabang GoChicken.
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div
          className="lg:col-span-2 bg-white shadow-lg rounded-xl p-4 sm:p-6"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
            <div>
              <h2 className="text-xl font-semibold text-gray-700 capitalize">
                Grafik {chartMode} Global
              </h2>
              <p className="text-sm text-gray-500">
                Visualisasi data finansial gabungan.
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex bg-gray-100 p-1 rounded-lg">
                ...
                >
                  <TrendingDown className="inline-block mr-1 h-4 w-4" />{" "}
                  Pengeluaran
                </button>
              </div>
              <div className="flex bg-gray-100 p-1 rounded-lg">
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <defs>
                    <linearGradient id="colorChart" x1="0" y1="0" x2="0" y2="1">
                      <stop
                        offset="5%"
                        stopColor={
                          chartMode === "pendapatan" ? "#ef4444" : "#f97316"
                        }
                        stopOpacity={0.8}
                      />
                      <stop
                  ...
                  <Area
                    ...
                    dataKey="value"
                    name={
                      chartMode === "pendapatan" ? "Pendapatan" : "Pengeluaran"
                    }
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <SummaryCard
              ...
              value={
                dashboardData.produk_tersedia
                  ? `${dashboardData.produk_tersedia} / ${dashboardData.total_produk}`
                  : "0 / 0"
              }
