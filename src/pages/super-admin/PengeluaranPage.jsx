...
import { Plus, Loader2, X, Calendar, Tag, FileText, ShoppingBag, Wallet, XCircle, Divide, Calculator } from "lucide-react";
    ...
    <>
      ...
      <motion.div className="space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Manajemen Pengeluaran</h1>
            <p className="text-gray-500 text-sm sm:text-base">Catat dan kelola semua pengeluaran untuk cabang</p>
          </div>
          <button
            ...
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto custom-scrollbar">
              <div className="p-6 space-y-4">
                ...
                {isPembelianBahanBaku ? (
                  <div className="pt-2">
                    <h3 className="text-md font-semibold text-gray-800 mb-2">Detail Pembelian Bahan Baku</h3>
                        ...
                        <div key={index} className="p-3 bg-gray-50 rounded-lg">
                          <div className="grid grid-cols-12 gap-x-3 gap-y-2 items-end">
                            <div className="col-span-12 sm:col-span-5">
                              <label className="text-xs font-medium text-gray-600">Bahan Baku</label>
                              <select 
                                name="id_bahan_baku" 
                            ...
                            <div className="col-span-6 sm:col-span-4">
                              ...
                              <p className="w-full p-2 border border-gray-300 rounded-lg cursor-default text-sm text-gray-900 bg-gray-50">
                                {bahanBakuList.find(b => b.id_bahan_baku == item.id_bahan_baku)?.harga_satuan ? formatRupiah(bahanBakuList.find(b => b.id_bahan_baku == item.id_bahan_baku).harga_satuan) : "-"}
                              </p>
                            </div>
                    ...
                    <div className="border border-gray-200 rounded-lg p-4 bg-gradient-to-r from-blue-50 to-indigo-50">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="flex items-center gap-2 flex-1">
                          ...
                          />
                          <label htmlFor="cicilanHarian" className="text-sm font-semibold text-gray-800 flex items-center gap-2">
                      ...
                      {isCicilanHarian && (
                        <div className="space-y-2 text-sm">
                          <p className="text-gray-600">
                            Total akan dibagi rata per hari dalam bulan <strong>{formData.tanggal ? new Date(formData.tanggal).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' }) : 'terpilih'}</strong>
                          </p>
                          <div className="grid grid-cols-2 gap-4 text-center">
                            <div className="bg-white rounded-lg p-3 border border-blue-200">
                              <p className="text-xs text-gray-500">Total Bulanan</p>
                              <p className="font-bold text-gray-800 text-lg">{displayJumlah}</p>
                            </div>
                            <div className="bg-white rounded-lg p-3 border border-green-200">
                              <p className="text-xs text-gray-500">Per Hari</p>
                              <p className="font-bold text-green-600 text-lg">{formatRupiah(dailyAmount)}</p>
                            </div>
                          </div>
                          <p className="text-xs text-gray-500 text-center">
                            Berdasarkan {getDaysInMonth(formData.tanggal)} hari dalam bulan ini
                          </p>
                        </div>
                      )}
                      ...
                      {!isCicilanHarian && (
                        <p className="text-xs text-gray-500">
                          Centang untuk membagi pengeluaran menjadi cicilan harian selama sebulan
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="flex items-center gap-2 text-gray-500">
                ...
                Tanggal: <strong className="text-gray-800">
                  {data?.tanggal ? format(new Date(data.tanggal), 'd MMMM yyyy') : 'N/A'}
                </strong>
              </div>
            </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="border rounded-lg overflow-hidden bg-white">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {data.details.map(d => (
                        <tr key={d.id_detail_pengeluaran} className="border-t text-gray-700 border-gray-200">
        ...
        >
          ...
          <div className="flex justify-center gap-3">
            ...
            <button 
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? <Loader2 className="animate-spin" size={18}/> : "Hapus"}
            </button>
          </div>
        </motion.div>
      </motion.div>
