...
import { UserPlus, Edit, Trash2, Plus, Loader2, Search, Filter, User, Building, Mail, XCircle, Shield } from "lucide-react";
...
const BranchAdminPage = () => {
    ...
    const filteredAdmins = admins.filter(admin => {
        const matchesSearch = admin.nama.toLowerCase().includes(searchTerm.toLowerCase()) ||
                            ...
                            (admin.cabang?.nama_cabang && admin.cabang.nama_cabang.toLowerCase().includes(searchTerm.toLowerCase()));
        const matchesCabang = !cabangFilter || (admin.cabang?.id_cabang === parseInt(cabangFilter));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingAdmin) {
                await axios.put(`${API_URL}/admin-cabang/${editingAdmin.id_user}`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setMessage({ type: "success", text: "Admin cabang berhasil diperbarui!" });
            } else {
                await axios.post(`${API_URL}/create-admin-cabang`, formData, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setMessage({ type: "success", text: "Admin cabang berhasil ditambahkan!" });
            }
            ...
            const errorMsg = err.response?.data?.message || "Gagal menyimpan admin cabang";
        ...
        <>
            ...
            <motion.div className="space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Kelola Admin Cabang</h1>
                        <p className="text-gray-500 text-sm sm:text-base">Kelola admin untuk setiap cabang toko</p>
                    </div>
                    <button
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            {[...new Set(admins.map(admin => admin.cabang?.id_cabang).filter(Boolean))].map(cabangId => {
                                const cabangData = admins.find(admin => admin.cabang?.id_cabang === cabangId)?.cabang;
                                return cabangData ? (
                                    <option key={cabangData.id_cabang} value={cabangData.id_cabang}>
                ...
                {!loading && !error && filteredAdmins.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500 mt-4">Tidak ada admin cabang ditemukan</p>
                    </div>
                )}
            </motion.div>
                        ...
                        >
                            <div className="p-6 border-b">
                                <h2 className="text-xl font-bold text-gray-800">
                                    {editingAdmin ? "Edit Admin Cabang" : "Tambah Admin Cabang Baru"}
                                </h2>
                            </div>
                                ...
                                <div>
                                    ...
                                    {cabang.length === 0 && (
                                        <p className="text-xs text-red-500 mt-1">
                                            Semua cabang sudah memiliki admin. Hapus admin cabang terlebih dahulu untuk menambahkan yang baru.
                                        </p>
                                    )}
                                </div>
            ...
            <AnimatePresence>
                {isDeleteModalOpen && deleteAdmin && (
                    ...
                    >
                        ...
                        >
                            <div className="p-6 border-b">
                                <h2 className="text-xl font-bold text-gray-800">Hapus Admin Cabang</h2>
                            ...
                            <div className="p-6">
                                <div className="flex items-center gap-4 p-4 bg-red-50 rounded-lg">
                                    ...
                                    <div>
                                        <p className="font-semibold text-gray-800">{deleteAdmin.nama}</p>
                                        <p className="text-sm text-gray-500">{deleteAdmin.email}</p>
                                        <p className="text-sm text-gray-500">{getCabangName(deleteAdmin)}</p>
                                    </div>
                                </div>
