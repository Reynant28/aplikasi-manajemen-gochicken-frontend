...
import { PlusCircle, Loader2, X, AlertTriangle, RefreshCw, User, Mail } from "lucide-react";
...
const KasirPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (selectedKasir) {
        res = await axios.put(`${API_URL}/kasir/${selectedKasir.id_user}`, apiData, { 
          headers: { Authorization: `Bearer ${token}` } 
        });
      } else {
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      const res = await axios.delete(`${API_URL}/kasir/${selectedKasir.id_user}`, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
  ...
  const renderContent = () => {
    ...
    return <KasirTable kasirList={kasirList} onEdit={openModal} onDelete={openConfirmDelete} />;
  };
    ...
    <>
      ...
      <motion.div className="space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Manajemen Kasir</h1>
            <p className="text-gray-500 text-sm sm:text-base">
              Kelola daftar kasir untuk cabang <strong>{cabang?.nama_cabang || 'N/A'}</strong>
            </p>
          </div>
          <motion.button 
            onClick={() => openModal()} 
          ...
          >
            ...
            >
              <div className="p-6 border-b flex justify-between items-center">
                ...
                <button onClick={closeModal} className="p-1 text-gray-400 hover:text-gray-700 rounded-full hover:bg-gray-100">
                  <X size={20} />
                </button>
              </div>
