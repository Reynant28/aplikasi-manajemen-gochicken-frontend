...
const ReportsPage = () => {
  ...
  const exportToPDF = async () => {
    ...
    setExporting(true);
    try {
      const blob = await pdf(<PDFReport reportData={reportData} filter={filter} cabang={cabang} />).toBlob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `Laporan-Penjualan-${cabang?.nama_cabang || 'Cabang'}-${filter === 'minggu' ? 'Minggu-Ini' : filter === 'bulan' ? 'Bulan-Ini' : 'Tahun-Ini'}-${new Date().toISOString().split('T')[0]}.pdf`;
      link.click();
  ...
  const PDFReport = ({ reportData, filter, cabang }) => {
    // Helper function to get sales trend data
        ...
        <Page style={styles.body}>
          ...
          <View style={styles.header}>
            <View style={styles.headerContent}>
              <Text style={styles.companyName}>{cabang?.nama_cabang || 'Cabang'}</Text>
          ...
          <View style={styles.footer}>
            <Text style={styles.footerText}>
              © {new Date().getFullYear()} {cabang?.nama_cabang || 'Cabang'} - Laporan ini dibuat secara otomatis
            </Text>
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        ...
        >
          ...
          <p className="text-gray-500 mt-1">Analisis performa penjualan untuk cabang {cabang?.nama_cabang || 'Anda'}.</p>
        </motion.div>
