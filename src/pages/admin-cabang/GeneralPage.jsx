...
const GeneralPage = () => {
  ...
  const [tersediaProduk, setTersediaProduk] = useState(0);
    ...
    const fetchStatsForCabang = async (id) => {
      try {
        ...
        if (res?.data?.status === "success" && !cancelled) {
          ...
          setTersediaProduk(d.produk_tersedia ?? 0);
  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      // Don't fetch if you are super-admin (or handle it with a global chart endpoint)
  ...
  }, [cabangId, token, user?.role, chartMode, chartFilter]); // Re-run when mode or filter changes
  ...
  useEffect(() => {
      ...
      const fetchRecentActivities = async () => {
          ...
          if (!user?.id_user) {
              setActivitiesError("User data not found. Please log in again.");
          ...
          try {
              ...
              const res = await axios.get(`${API_URL}/dashboard/user/activities`, {
                  headers: { Authorization: `Bearer ${token}` },
              });

              if (res?.data?.status === 'success' && !cancelled) {
                  // Assuming the API returns an array of activities
      ...
      if (token && user?.id_user) {
          ...
          setActivitiesLoading(false); // Stop loading if no user is found
      }

      return () => {
          cancelled = true;
      };
  }, [token, user?.id_user]); // Re-run when user or token changes
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          >
              ...
              <div className="flex flex-col">
                  <h2 className="text-sm font-medium text-gray-500">Total Produk Tersedia</h2>
                  {loading ? (
                      <p className="text-2xl font-bold text-gray-800 animate-pulse">...</p>
                  ) : (
                      <p className="text-2xl font-bold text-green-600">
                          {tersediaProduk} / {totalProduk}
                      </p>
                  )}
              </div>
          </motion.div>
