...
const processChartData = (rawData = [], filter) => {
  ...
  if (filter === "minggu") {
    const days = ["Min", "Sen", "Sel", "Rab", "Kam", "Jum", "Sab"].map(
      (name) => ({ name, value: 0 })
    );
...
const GeneralPage = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      setError(null);
      try {
        const res = await axios.get(`${API_URL}/dashboard/cabang/${cabangId}`, {
        ...
        if (res.data.data.revenue_breakdown) {
          const processedData = processRevenueBreakdown(
            res.data.data.revenue_breakdown
          );
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-500 text-sm sm:text-base">
          Ringkasan performa untuk cabang{" "}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Visualisasi data finansial berdasarkan waktu.
              </p>
            </div>
          ...
          <div className="w-full h-80">
            {chartLoading ? (
              ...
              <div className="flex justify-center items-center h-full text-red-500">
                <AlertCircle className="w-6 h-6 mr-2" /> {chartError}
              </div>
            ) : chartData.length === 0 ? (
              <div className="flex justify-center items-center h-full text-gray-500">
                <BarChart3 className="w-6 h-6 mr-2" /> Tidak ada data.
              </div>
            ) : (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <SummaryCard
              ...
              value={`${dashboardData.produk_tersedia ?? "0"} / ${
                dashboardData.total_produk ?? "0"
              }`}
