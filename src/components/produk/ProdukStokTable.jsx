            ...
            {paginatedProduk.map((prod, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`bg-white rounded-xl shadow-md border overflow-hidden flex flex-col transition-all duration-300 ${
                    hasPendingChange
                ...
                >
                  <div className="relative group">
                    <img
                      src={
                        prod.gambar_url ||
                        "https://placehold.co/400x300/e2e8f0/64748b?text=N/A"
                      }
                      alt={prod.nama_produk}
                      className="w-full h-40 object-cover transition-all duration-300 group-hover:brightness-75"
                    />
                    <button
                      onClick={() => onImageClick(prod.gambar_url)}
                      className="absolute inset-0 flex items-center justify-center transition-all duration-300 bg-black/20 opacity-0 group-hover:opacity-100"
                    >
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 flex-grow flex flex-col justify-end">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() =>
                              onStageChange(prod.id_stock_cabang, -1)
                            }
                            disabled={displayStok <= 0}
                            className="p-2 text-red-600 hover:bg-red-100 disabled:text-gray-300 disabled:hover:bg-transparent rounded-full transition-colors"
                          >
                            <MinusCircle size={24} />
                          </button>
                          <button
