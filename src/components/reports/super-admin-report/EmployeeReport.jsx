...
import { Loader2, ServerCrash, Search, AlertTriangle, User, Phone, MapPin, Building2 } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const employeesWithCabang = (employeesRes.data.data || []).map(employee => ({
          ...employee,
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(item => {
      const matchesSearch = 
        ...
        (item.cabang && item.cabang.toLowerCase().includes(searchTerm.toLowerCase()));
      
      const matchesCabang = selectedCabang === 'all' || 
