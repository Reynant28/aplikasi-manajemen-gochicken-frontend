    ...
    const fetchData = async () => {
      ...
      try {
        const [transaksiRes, pengeluaranRes] = await Promise.all([
          axios.get(
            `${API_URL}/reports/sales/transactions?page=${transaksiPage}&limit=${itemsPerPage}&filter=${filter}`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
          axios.get(
            `${API_URL}/reports/sales/expenses?page=${pengeluaranPage}&limit=${itemsPerPage}&filter=${filter}`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
        ]);
