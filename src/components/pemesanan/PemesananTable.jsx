...
const PaymentIcon = ({ method }) => {
  switch (method?.toLowerCase()) {
    case 'tunai': 
      return <Wallet size={14} className="mr-2 text-gray-500" />;
    case 'qris': 
      return <QrCode size={14} className="mr-2 text-gray-500" />;
    case 'debit': 
      return <CreditCard size={14} className="mr-2 text-gray-500" />;
    default: 
      return null;
  }
};
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-gray-500 transition text-gray-900 placeholder:text-gray-400 bg-white"
        />
      </div>
              ...
              {filteredData.length > 0 ? (
                filteredData.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-800 text-right">
                      {formatRupiah(p.total_harga)}
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-center">
                      <StatusBadge status={p.status_transaksi} />
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex justify-center gap-1 sm:gap-2">
                        <button 
                          onClick={() => onView(p)} 
                          className="p-2 text-blue-600 hover:text-blue-800 transition-colors rounded-full hover:bg-blue-50 border border-transparent hover:border-blue-100"
                          title="Lihat Detail"
                        >
                          <Eye size={16} />
                        </button>
                        <button 
                          onClick={() => onEdit(p)} 
                          className="p-2 text-yellow-600 hover:text-yellow-800 transition-colors rounded-full hover:bg-yellow-50 border border-transparent hover:border-yellow-100"
                          title="Edit Status"
                        >
                          <Edit size={16} />
                        </button>
                        <button 
                          onClick={() => onDelete(p)} 
                          className="p-2 text-red-600 hover:text-red-800 transition-colors rounded-full hover:bg-red-50 border border-transparent hover:border-red-100"
                          title="Hapus"
                        >
                          <Trash2 size={16} />
                        </button>
                ...
                <tr>
                  <td colSpan="6" className="text-center py-12">
                    <div className="flex flex-col items-center gap-3 text-gray-500">
                      ...
                      <p className="font-semibold">Tidak ada data pemesanan.</p>
                    </div>
                  </td>
                </tr>
              )}
